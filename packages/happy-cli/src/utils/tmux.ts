        ...
        try {
            ...
            if (env && Object.keys(env).length > 0) {
                for (const [key, value] of Object.entries(env)) {
                    ...
                    // Pass env var as KEY=VALUE without shell-style quoting.
                    // executeTmuxCommand() uses spawn() with shell:false, so each
                    // element of the args array is passed directly to tmux as a
                    // single argv entry. Wrapping the value in double quotes would
                    // cause tmux to include the literal quote characters in the
                    // value (e.g. HAPPY_SERVER_URL='"http://..."' instead of
                    // HAPPY_SERVER_URL='http://...'), breaking URL parsing and auth.
